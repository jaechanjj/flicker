...
public class MovieController {
    ...
    // 전체 영화 목록 조회
    @GetMapping("/list")
    public ResponseEntity<ResponseDto> getAllMovieList() {
        List<MovieListResponse> response = movieService.getAllMovieList();
        return ResponseDto.response(StatusCode.SUCCESS, response);
    }

    // 장르별 영화 목록 조회
    @GetMapping("/list/genre/{genre}")
    public ResponseEntity<ResponseDto> getMovieListByGenre(@PathVariable String genre) {
        List<MovieListResponse> response = movieService.getMovieListByGenre(genre);
        return ResponseDto.response(StatusCode.SUCCESS, response);
    }

    // 배우별 영화 목록 조회
    @GetMapping("/list/actor/{actorName}")
    public ResponseEntity<ResponseDto> getMovieListByActor(@PathVariable String actorName) {
        List<MovieListResponse> response = movieService.getMovieListByActor(actorName);
        return ResponseDto.response(StatusCode.SUCCESS, response);
    }

    // 검색 영화 목록 조회
    @GetMapping("/list/search/{keyword}/{userSeq}")
    public ResponseEntity<ResponseDto> getMovieListByKeyword(@PathVariable String keyword, @PathVariable int userSeq) {
        List<MovieListResponse> response = movieService.getMovieListByKeyword(keyword, userSeq);
        return ResponseDto.response(StatusCode.SUCCESS, response);
    }
